  ...
  var glUtils = {
    ...
    checkWebGL: function(canvas){
      ...
      for(var i = 0; i < contexts.length; i++){
        try{gl = canvas.getContext(contexts[i]);}
        catch (e) {}
        if(gl){break;}
      }
      if(!gl){alert("WebGL not available, sorry! Please use a new version of Chrome or Firefox.");}
      return gl;
    },
    ...
    SL: {
      ...
      checkForComplete: function(){
        if(!this.slShaderCount){this.loadedSignal.dispatch();}
      },
